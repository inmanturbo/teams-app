<x-jet-dialog-modal wire:model="creatingNewDatabase">
    ...
    <x-slot name="content">
        ...
        <div class="inline-flex flex-row justify-between w-full sm:col-span-4">
            <x-jet-label class="w-12 mt-4 mr-2" for="type" value="{{ __('Driver') }}" />

            {{-- @json($state) --}}
            <x-select id="type" class="block w-full mt-1" wire:model="state.driver">
                <option value="sqlite" >Sqlite</option>
                <option value="mysql" >MySQL</option>
            </x-select>

            <x-jet-input-error for="driver" class="w-full mt-2" />
        </div>
