...
class CreateDatabase implements CreatesDatabase
{
    ...
    public function create($user, array $input)
    {
        $nameRules = [
            ...
            'max:255',
            'min:3',
            'unique:' . config('landlord.db_connection') . '.team_databases',
        ...
        Validator::make($input, [
            'name' => $nameRules,
            'driver' => ['nullable', 'string', 'in:sqlite,mysql'],
            'database_uuid' => ['nullable', 'unique:' . config('landlord.db_connection') . '.team_databases,uuid'],
        ])->validateWithBag('createDatabase');

        $uuid = $input['database_uuid'] ?? Str::uuid();

        $aggregate = TeamDatabaseAggregate::retrieve($uuid)
            ->createTeamDatabase($user->uuid, name: $input['name'], driver: $input['driver'] ?? null)
            ->persist();
